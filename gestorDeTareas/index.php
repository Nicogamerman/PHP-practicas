<?php
    ...
    if(file_exists("archivo.txt")){ //si el archivo existe lo abrimos.
        ...
        //si el archivo no existe creamos uno
        $aTareas = array();

    }   
        //variable de la query string ID, si no viene la defino, sino la defino como vacia
    if(isset($_GET["id"])){
        $id = $_GET["id"];
    } else {
        $id="";
    }  
        //definimos todas las variables donde van a estar almacenados los datos
    if($_POST){
        ...
        if($id >= 0){ //si el ID es mayor o igual a 0, estoy editando una tarea            
            $aTareas[$id] = array(
            ...
            //Sino estoy agregando una tarea
            $aTareas[] = array(
                "fecha" => date("d/m/Y"),
                "prioridad" => $prioridad,
                "usuario" => $usuario,
